...
var Hi = _interopRequireWildcard(require("react-icons/hi"));
...
function convertFormat(name, options) {
  ...
  var separator = "-"; //Remove react icon prefixes/identifiers Hi

  name = name.replace(/^(Hi)(.*$)/, "$2"); //Separate letters followed by numbers (decamelize defaults to omitting separation of letter followed by number)
...
function _default(provider) {
  return function () {
    ...
    var icons = Object.keys(Hi).map(function (name) {
      var Icon = Hi[name];
      return {
        provider: provider,
